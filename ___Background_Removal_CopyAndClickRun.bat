...
set /p "fuzz=Nhập giá trị xóa nền từ 0->99 (giá trị càng lớn càng xóa nền nhiều, không sử dụng chọn 0): "
set /p "lowLev=Nhập giá trị tăng đậm hình từ 0->99 (giá trị càng lớn hình càng đậm, không sử dụng chọn 0): "
...
@echo on
for %%i in (*.tif *.png *.jpg *.jpeg) do convert "%%i" -fuzz "%fuzz%%%" -fill White -opaque White -level %lowLev%%%,100%% -sigmoidal-contrast %scl%,%sch%%% -black-threshold %bt%%% "%outdir%\%%i"
